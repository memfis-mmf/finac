<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
    <style>
    @page {
        margin: 0cm, 0cm;
    }

    html,
    body {
        padding: 0;
        margin: 0;
        font-size: 12px;
    }

    body {
        margin-top: 4cm;
        margin-bottom: 1cm;
    }

    header {
        position: fixed;
        top: 0cm;
        left: 0cm;
        right: 0cm;
        height: 3cm;
    }

    footer {
        position: fixed;
        bottom: 0cm;
        left: 0cm;
        right: 0cm;
        height: 1.8cm;
    }

    ul li {
        display: inline-block;
    }

    table {
        border-collapse: collapse;
    }

    #head {
        top: 20px;
        left: 510px;
        position: absolute;
        color: #5c5b5b;
    }

    .container {
        width: 100%;
        margin: 0 36px;
    }

    .barcode {
        margin-left: 70px;
        margin-top: 12px;
    }

    #content {
        width: 100%;
        margin-bottom: 75px;
    }

    .page_break {
        page-break-before: always;
    }
    </style>
</head>

<body>
    <header>
        <img src="./vendor/courier/img/form/trial-balance/Header.png" alt="" width="100%">
        <div id="head">
            <div style="margin-right:20px;text-align:center;">
                <h1 style="font-size:24px;">PROFIT & LOSS</h1>
                <h4>Date Period d-m-y - d-m-y</h4>
            </div>
        </div>
    </header>

    <footer>
        <table width="100%">
            <tr>
                <td> <span style="margin-left:6px;">Printed By : </span> </td>
            </tr>
        </table>
        <img src="./vendor/courier/img/form/trial-balance/Footer.png" width="100%" alt="">
    </footer>

    <div id="content">
        <div class="container">
            <table width="100%" cellpadding="5" >
                <tr style="background:#5f6b5e; color:white;font-weight: bold;">
                    <td width="80%">Account Name</td>
                    <td width="10%" align="right">Accumulated</td>
                    <td width="10%" align="right">Period</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:2px solid black; font-size:15px">REVENUE
                    </td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                </tr>
                <tr>
                    <td width="20%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">OPERATING
                        REVENUE</h4>
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">MMF - Heavy Maintenance Revenue</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">MMF - Workshop Revenue</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">MMF - Rental Hangar Revenue</td>
                    <td width="20%" align="right">1000.000.000</td>
                    <td width="20%" align="right">1000.000</td>
                </tr>
                <tr>
                    <td width="60%">MMF - Other Revenue</td>
                    <td width="20%" align="right">2500.000</td>
                    <td width="20%" align="right">2500.000</td>
                </tr>
                <tr>
                    <td width="60%">MMF - Workshop Jakarta</td>
                    <td width="20%" align="right">2500.000</td>
                    <td width="20%" align="right">2500.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">SALES
                        DISCOUNT</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Sales Discount</td>
                    <td width="20%" align="right">3000.000</td>
                    <td width="20%" align="right">3000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:2px solid black; font-size:15px"><br>NON
                        OPERATING REVENUE (EXPENSE)</td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">NON
                        OPERATING REVENUE</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Interest Income</td>
                    <td width="20%" align="right">3000.000</td>
                    <td width="20%" align="right">3000.000</td>
                </tr>
                <tr>
                    <td width="60%">Exchange Rate Gap Income</td>
                    <td width="20%" align="right">500.000.000</td>
                    <td width="20%" align="right">500.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Other Income</td>
                    <td width="20%" align="right">500.000.000</td>
                    <td width="20%" align="right">500.000.000</td>
                </tr>
                <tr style="background:#add8f7;font-weight: bold; font-size:12px;">
                    <td width="60%">TOTAL REVENUE</td>
                    <td width="20%" align="right">2.890.000.000</td>
                    <td width="20%" align="right">2.890.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:2px solid black; font-size:15px">
                        <br> COST OF GOLD</td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">PRODUCTION
                        EXPENSES</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Depreciation Expense</td>
                    <td width="20%" align="right">1200.000</td>
                    <td width="20%" align="right">1200.000</td>
                </tr>
                <tr>
                    <td width="60%">Insurance Expanses</td>
                    <td width="20%" align="right">1200.000</td>
                    <td width="20%" align="right">1200.000</td>
                </tr>
                <tr>
                    <td width="60%">Fuel Expenses</td>
                    <td width="20%" align="right">1200.000</td>
                    <td width="20%" align="right">1200.000</td>
                </tr>
                <tr>
                    <td width="60%">Overhaul Expenses</td>
                    <td width="20%" align="right">1200.000</td>
                    <td width="20%" align="right">1200.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">MAINTENANCE
                        & REPAIR EXPENSES</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Maintenance & Repair Expenses</td>
                    <td width="20%" align="right">4000.000</td>
                    <td width="20%" align="right">4000.000</td>
                </tr>
                <tr>
                    <td width="60%">Maintenance & Repair Expenses Jakarta</td>
                    <td width="20%" align="right">4000.000</td>
                    <td width="20%" align="right">4000.000</td>
                </tr>
                <tr>
                    <td width="60%">Joint Operation & Third Party Expense</td>
                    <td width="20%" align="right">7000.000</td>
                    <td width="20%" align="right">7000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">DIRECT
                        LABOR</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Salary - Technicians</td>
                    <td width="20%" align="right">7000.000</td>
                    <td width="20%" align="right">7000.000</td>
                </tr>
                <tr>
                    <td width="60%">Allowance & Overtime - Technicians</td>
                    <td width="20%" align="right">7000.000</td>
                    <td width="20%" align="right">7000.000</td>
                </tr>
                <tr>
                    <td width="60%">Duty Trip Expense - Technicians</td>
                    <td width="20%" align="right">7000.000</td>
                    <td width="20%" align="right">7000.000</td>
                </tr>
                <tr>
                    <td width="60%">Training Expense</td>
                    <td width="20%" align="right">7000.000</td>
                    <td width="20%" align="right">7000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        DISTRIBUTION COST</td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Expedition, Freight & Inclaring</td>
                    <td width="20%" align="right">5000.000</td>
                    <td width="20%" align="right">5000.000</td>
                </tr>
                <tr>
                    <td width="60%">Landing Fees & Route Charges</td>
                    <td width="20%" align="right">5000.000</td>
                    <td width="20%" align="right">5000.000</td>
                </tr>
                <tr>
                    <td width="60%">Ground Handling</td>
                    <td width="20%" align="right">5000.000</td>
                    <td width="20%" align="right">5000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:2px solid black; font-size:15px;">
                        <br> OPERATING EXPENSES
                    </td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        SALES COST
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Commision</td>
                    <td width="20%" align="right">6700.000</td>
                    <td width="20%" align="right">6700.000</td>
                </tr>
                <tr>
                    <td width="60%">Promotion</td>
                    <td width="20%" align="right">6700.000</td>
                    <td width="20%" align="right">6700.000</td>
                </tr>
                <tr>
                    <td width="60%">Salary - Sales</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Allowances & Overtime - Sales</td>
                    <td width="20%" align="right">4000.000</td>
                    <td width="20%" align="right">4000.000</td>
                </tr>
                <tr>
                    <td width="60%">Duty Trip Expense - Sales</td>
                    <td width="20%" align="right">4000.000</td>
                    <td width="20%" align="right">4000.000</td>
                </tr>
                <tr>
                    <td width="60%">Training Expenses - Sales</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        ORGANIZATION EXPENSE
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Allowances & Overtime - Administration</td>
                    <td width="20%" align="right">6000.0000</td>
                    <td width="20%" align="right">6000.0000</td>
                </tr>
                <tr>
                    <td width="60%">Salary - Administration</td>
                    <td width="20%" align="right">6000.0000</td>
                    <td width="20%" align="right">6000.0000</td>
                </tr>
                <tr>
                    <td width="60%">Duty Trip Expense - Administration</td>
                    <td width="20%" align="right">80.000.000</td>
                    <td width="20%" align="right">80.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Training & Recruitment Exp. - Administration</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        GENERAL EXPENSE
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%"><b>Maintenance & Repair Expense</b></td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Maintenance & Repair Vehicle</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Maintenance & Repair Building</td>
                    <td width="20%" align="right">10.000.000</td>
                    <td width="20%" align="right">10.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Maintenance & Repair Equipment</td>
                    <td width="20%" align="right">10.000.000</td>
                    <td width="20%" align="right">10.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Rental Expenses</td>
                    <td width="20%" align="right">10.000.000</td>
                    <td width="20%" align="right">10.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        OFFICE COST
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Electricity, Water, Gas, Telephone</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Supplies Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Office Operation</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Outsourcing Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Research & Development Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Office Operation Jakarta</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Public Relation Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Tax Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        DEPRECIATION & AMORTIZATION EXPENSE
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Depreciation Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Amortization Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        OTHER EXPENSE
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Removing Receivable Expanses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:2px solid black; font-size:15px"><br>
                        NON OPERATING EXPENSES
                    </td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:2px solid black"></td>
                </tr>
                <tr>
                    <td width="60%" style="font-weight: bold; border-bottom:1px solid black; font-size:12px">
                        NON OPERATING EXPENSES MMF
                    </td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                    <td width="20%" align="right" style=" border-bottom:1px solid black"></td>
                </tr>
                <tr>
                    <td width="60%">Bank & Non Bank Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Exchange Rate Gap Income</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Pinalty & Tax Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr>
                    <td width="60%">Others Expenses</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                    <td width="20%" align="right">20.000.000.000.000</td>
                </tr>
                <tr style="background:#add8f7;font-weight: bold; font-size:12px;">
                    <td width="60%">
                        TOTAL REVENUE
                    </td>
                    <td width="20%" align="right">40.000.000.000.000</td>
                    <td width="20%" align="right">40.000.000.000.000</td>
                </tr>

            </table>

            <table width="100%" cellpadding="5" style="margin-top:28pt" >
                <tr >
                    <td width="10%"></td>
                    <td width="30%" style="font-weight: bold; font-size:13px;">
                        CALCULATED RETURN
                    </td>
                    <td width="10%" align="right"style="font-weight: bold; font-size:13px;">30.028.540.000.000</td>
                    <td width="10%" align="right"style="font-weight: bold; font-size:13px;">30.028.540.000.000</td>
                </tr>
            </table>

        </div>
    </div>
</body>

</html>